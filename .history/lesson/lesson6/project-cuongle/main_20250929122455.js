...
(async () => {
  try {
    ...
    const phones = all.filter(p => (p.category || "").trim().toLowerCase() === "điện thoại");
    render(phones, elPhones, { limit: 4 });

    // Laptop: chỉ lấy 4 sản phẩm
    const laptops = all.filter(p => (p.category || "").trim().toLowerCase() === "laptop");
    render(laptops, elLaptops, { limit: 4 });

  } catch (err) {
    console.error(err);
    elFeatured.innerHTML = elPhones.innerHTML = elLaptops.innerHTML =
